...
import { BtnIncepeProgram, BtnIncepePauza, BtnIncheieProgram, BtnIncheiePauza } from './PontajButtons';
...
function PontajController(){
    const { pontajState, setPontajState} : any = useContext(PontajContext)
    const { account }: any = useContext(AccountContext)
    const [citat, setCitat] = useState<string>("");

    useEffect(()=> setCitat(AfiseazaCitatRandom()), [])

    function trimiteStatusActivitate(_codOperatiune: number, _pontajState: number){
        if(navigator.geolocation){
    
            navigator.geolocation.getCurrentPosition((position)=>HandleSend(position, _codOperatiune, _pontajState), showGelocationError)
        } 
        else alertError("A aparut o eroare de la geolocatie.")
    }

    function HandleSend(position:any,_codOperatiune: number, _pontajState: number){
    ...
    function ButtonDOM(){
        ...
        else if (pontajState === 0) return <BtnIncepeProgram onClick={() => trimiteStatusActivitate(1, 1)}/>
        else if(pontajState === 1) return (
            <>
